...
class OrderValidatorApp:
    def __init__(self, root):
        ...
        # Run button
        self.run_button = ttk.Button(self.top_frame, text="Validate & Fix Data", command=self.start_validation)
    ...
    def start_validation(self):
        ...
        thread = threading.Thread(target=self.run_validation, args=(input_file,))
        thread.daemon = True
        thread.start()
    
    def run_validation(self, input_file):
        try:
            # Get the output path (same as input but with timestamp)
            base_name = os.path.splitext(input_file)[0]
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            output_excel = f"{base_name}_{timestamp}.xlsx"
            output_txt = f"{base_name}_{timestamp}.txt"
            ...
            with open(output_txt, 'w', encoding='utf-8') as f:
                ...
                try:
                    ...
                    if output:
                        ...
                        self.status_var.set("Error during validation. Check console for details.")
